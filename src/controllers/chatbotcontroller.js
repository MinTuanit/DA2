...
async function buildPrompt() {
  const nowPlayingMovies = await Movie.find({ status: "Now Playing" }).select(
    "title"
  );
  const comingSoonMovies = await Movie.find({ status: "Coming Soon" }).select(
    "title"
  );
